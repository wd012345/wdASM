;file:          vmul64fp256.asm
;
; Assembly function that multiplies 64 bit floating point components of two
...
; extern(C) double * vmul64fp256(<arg_list>);
;
; where <arg_list> is:
...
loop1:
    ...
    vmulpd      ymm0,ymm1,[rdx+rax]             ; process looped stage 1
    vmulpd      ymm2,ymm3,[rdx+rax+32]
    vmulpd      ymm4,ymm5,[rdx+rax+64]
    vmulpd      ymm6,ymm7,[rdx+rax+96]
    vmulpd      ymm8,ymm9,[rdx+rax+128]
    vmulpd      ymm10,ymm11,[rdx+rax+160]
    vmulpd      ymm12,ymm13,[rdx+rax+192]
    vmulpd      ymm14,ymm15,[rdx+rax+224]
...
loop2:
    vmovapd     ymm1,[rsi+rax]                  ; load first operands
    vmulpd      ymm0,ymm1,[rdx+rax]             ; process looped stage 2
    vmovapd     [rcx+rax],ymm0                  ; write results
    add         rax,32                          ; dA == 8N_f
    add         rbx,4                           ; N_f += 4 components
    cmp         rdi,rbx                         ; N >= N_f ?
...
loop3:
    ...
    vmulpd      xmm0,xmm1,[rdx+rax]             ; process looped stage 3
...
loop4:
    ...
    mulsd       xmm0,[rdx+rax]                  ; process looped stage 4
