;file:          vadd64fp256.asm
;
; Assembly function that adds 64 bit floating point components of two vectors
; with N components according to:
;     w_1 = u_1 + v_1
;     w_2 = u_2 + v_2
;           .
;           .
...
; extern(C) double * vadd64fp256(<arg_list>);
;
; where <arg_list> is:
...
loop1:
    ...
    vaddpd      ymm0,ymm1,[rdx+rax]             ; process looped stage 1
    vaddpd      ymm2,ymm3,[rdx+rax+32]
    vaddpd      ymm4,ymm5,[rdx+rax+64]
    vaddpd      ymm6,ymm7,[rdx+rax+96]
    vaddpd      ymm8,ymm9,[rdx+rax+128]
    vaddpd      ymm10,ymm11,[rdx+rax+160]
    vaddpd      ymm12,ymm13,[rdx+rax+192]
    vaddpd      ymm14,ymm15,[rdx+rax+224]
...
loop2:
    vmovapd     ymm1,[rsi+rax]                  ; load first operands
    vaddpd      ymm0,ymm1,[rdx+rax]             ; process looped stage 2
    vmovapd     [rcx+rax],ymm0                  ; write results
    add         rax,32                          ; dA == 8N_f
    add         rbx,4                           ; N_f += 4 components
    cmp         rdi,rbx                         ; N >= N_f ?
...
loop3:
    ...
    vaddpd      xmm0,xmm1,[rdx+rax]             ; process looped stage 3
...
loop4:
    ...
    addsd       xmm0,[rdx+rax]                  ; process looped stage 4
