;file:          vmul32fp256.asm
;
; Assembly function that multiplies 32 bit floating point components of two
...
; extern(C) float * vmul32fp256(<arg_list>);
;
; where <arg_list> is:
...
loop1:
    ...
    vmulps      ymm0,ymm1,[rdx+rax]             ; process looped stage 1
    vmulps      ymm2,ymm3,[rdx+rax+32]
    vmulps      ymm4,ymm5,[rdx+rax+64]
    vmulps      ymm6,ymm7,[rdx+rax+96]
    vmulps      ymm8,ymm9,[rdx+rax+128]
    vmulps      ymm10,ymm11,[rdx+rax+160]
    vmulps      ymm12,ymm13,[rdx+rax+192]
    vmulps      ymm14,ymm15,[rdx+rax+224]
...
loop2:
    vmovaps     ymm1,[rsi+rax]                  ; load first operands
    vmulps      ymm0,ymm1,[rdx+rax]             ; process looped stage 2
    vmovaps     [rcx+rax],ymm0                  ; write results
    add         rax,32                          ; dA == 4N_f
    add         rbx,8                           ; N_f += 8 components
    cmp         rdi,rbx                         ; N >= N_f ?
...
loop3:
    ...
    vmulps      xmm0,xmm1,[rdx+rax]             ; process looped stage 3
...
loop4:
    ...
    mulss       xmm0,[rdx+rax]                  ; process looped stage 4
