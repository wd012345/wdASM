;file:          vsub64fp256.asm
;
; Assembly function that subtracts 64 bit floating point components of two
...
; extern(C) double * vsub64fp256(<arg_list>);
;
; where <arg_list> is:
...
loop1:
    ...
    vsubpd      ymm0,ymm1,[rdx+rax]             ; process looped stage 1
    vsubpd      ymm2,ymm3,[rdx+rax+32]
    vsubpd      ymm4,ymm5,[rdx+rax+64]
    vsubpd      ymm6,ymm7,[rdx+rax+96]
    vsubpd      ymm8,ymm9,[rdx+rax+128]
    vsubpd      ymm10,ymm11,[rdx+rax+160]
    vsubpd      ymm12,ymm13,[rdx+rax+192]
    vsubpd      ymm14,ymm15,[rdx+rax+224]
...
loop2:
    vmovapd     ymm1,[rsi+rax]                  ; load first operands
    vsubpd      ymm0,ymm1,[rdx+rax]             ; process looped stage 2
    vmovapd     [rcx+rax],ymm0                  ; write results
    add         rax,32                          ; dA == 8N_f
    add         rbx,4                           ; N_f += 4 components
    cmp         rdi,rbx                         ; N >= N_f ?
...
loop3:
    ...
    vsubpd      xmm0,xmm1,[rdx+rax]             ; process looped stage 3
...
loop4:
    ...
    subsd       xmm0,[rdx+rax]                  ; process looped stage 4
