;file:          vdiv32fp256.asm
;
; Assembly function that divides 32 bit floating point components of two
...
; extern(C) float * vdiv32fp256(<arg_list>);
;
; where <arg_list> is:
...
loop1:
    ...
    vdivps      ymm0,ymm1,[rdx+rax]             ; process looped stage 1
    vdivps      ymm2,ymm3,[rdx+rax+32]
    vdivps      ymm4,ymm5,[rdx+rax+64]
    vdivps      ymm6,ymm7,[rdx+rax+96]
    vdivps      ymm8,ymm9,[rdx+rax+128]
    vdivps      ymm10,ymm11,[rdx+rax+160]
    vdivps      ymm12,ymm13,[rdx+rax+192]
    vdivps      ymm14,ymm15,[rdx+rax+224]
...
loop2:
    vmovaps     ymm1,[rsi+rax]                  ; load first operands
    vdivps      ymm0,ymm1,[rdx+rax]             ; process looped stage 2
    vmovaps     [rcx+rax],ymm0                  ; write results
    add         rax,32                          ; dA == 4N_f
    add         rbx,8                           ; N_f += 8 components
    cmp         rdi,rbx                         ; N >= N_f ?
...
loop3:
    ...
    vdivps      xmm0,xmm1,[rdx+rax]             ; process looped stage 3
...
loop4:
    ...
    divss       xmm0,[rdx+rax]                  ; process looped stage 4
